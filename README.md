...
1. Explain the difference between `RDBMS` and `SQL`.
  Regional Data Base Management System is what RDBMS means and it allows you to manage data effectivly in the back end, while SQL is the language used to manage that said database.

1. Why do tables need a `primary key`?
  on a table everything needs to be organized and they way to do that is with a numbered system that is what a primary key is used for to find and use data effectivly.

1. What is the name given to a table column that references the primary key on another table.
  foreign key

1. What do we need in order to have a _many to many_ relationship between two tables.
  a seperate table of foregin keys from the two tables in order to link them together cohesively so that we can group things together
